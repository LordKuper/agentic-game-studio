...
- **Design changes:** "Run `/ags-propagate-design-change [path]` on the affected design document to find downstream impacts before making changes."
- **Balance adjustments:** "Run `/ags-balance-check [system]` to verify the full balance picture before tuning values."
- **Bugs:** "Use `/ags-bug-report` to formally track these."
- **Polish items:** "Add to the polish backlog in `.ags/project/` when the team reaches that phase."
...
Ask: "May I write this playtest report to `.ags/project/qa/playtests/playtest-[date]-[tester].md`?"
...
- After addressing design changes: re-run `/ags-design-review` on the updated GDD.
- After fixing bugs: re-run `/ags-bug-triage` to update priorities.
...
2. Persist current draft to `.ags/project/reviews/.tmp/playtest-report-[id]-iter[N]-draft.md`.
3. **Spawn in one message, in parallel**:
   - All internal reviewer Tasks (qa-lead + game-designer).
