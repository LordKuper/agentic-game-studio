...
Fast situational awareness check, not sprint review. Reads current sprint plan
and story files, scans for status markers, produces concise snapshot in under
30 lines. Detailed sprint management → `/ags-sprint-plan update` or `/ags-milestone-review`.

**Read-only.** Never proposes changes, never asks to write files, at most one
concrete recommendation per run.
...
- If an argument is given (e.g., `/ags-sprint-status 3`), search
...
- If `.ags/project/sprints/` does not exist or is empty, report: "No sprint
  files found. Start a sprint with `/ags-sprint-plan new`." Then stop.
...
**First: check for `.ags/project/ags-sprint-status.yaml`.**
...
"⚠ No `sprint-status.yaml` found — status inferred from markdown. Run `/ags-sprint-plan update` to generate one."
...
time remaining. Recommend replanning with `/ags-sprint-plan update`.
```
...
Run `/ags-story-readiness sprint` to validate story file coverage.
```
...
Read-only. Reports observed facts from disk.

- Does not update sprint plan
- Does not change story status
- Does not propose scope cuts (use `/ags-sprint-plan update`)
- At most one recommendation per run

More detail on specific story → read file directly or `/ags-story-readiness [path]`.
Sprint replanning → `/ags-sprint-plan update`.
End-of-sprint retrospective → `/ags-milestone-review`.
