---
name: dev-story
description: "Read a story file and implement it. Loads the full context (story, GDD requirement, ADR guidelines, control manifest), routes to the right programmer agent for the system and engine, implements the code and test, and confirms each acceptance criterion. The core implementation skill вЂ” run after /story-readiness, before /code-review and /story-done."
...
drives implementation to completion вЂ” including writing the test.

**The loop for every story:**
```
/qa-plan sprint           в†ђ define test requirements before sprint begins
/story-readiness [path]   в†ђ validate before starting
/dev-story [path]         в†ђ implement it  (this skill)
/code-review [files]      в†ђ review it
/story-done [path]        в†ђ verify and close it
```
...
|------|------|------------|
| TR registry | `design/architecture/tr-registry.yaml` | **STOP** вЂ” "TR registry not found. Run `/create-epics` to generate it." |
| Governing ADR | path from story's ADR field | **STOP** вЂ” "ADR file [path] not found. Run `/architecture-decision` to create it, or correct the filename in the story's ADR field." |
| Control manifest | `design/architecture/control-manifest.md` | **WARN and continue** вЂ” "Control manifest not found вЂ” layer rules cannot be checked. Run `/create-control-manifest`." |
...
Read all of the following simultaneously вЂ” these are independent reads. Do not start implementation until all context is loaded:

### The story file
Extract and hold:
- **Story title, ID, layer, type** (Logic / Integration / Visual/Feel / UI / Config/Data)
- **TR-ID** вЂ” the GDD requirement identifier
- **Governing ADR** reference
- **Manifest Version** embedded in story header
- **Acceptance Criteria** вЂ” every checkbox item, verbatim
- **Implementation Notes** вЂ” the ADR guidance section in the story
- **Out of Scope** boundaries
- **Test Evidence** вЂ” the required test file path
- **Dependencies** вЂ” what must be DONE before this story

### The TR registry
Read `design/architecture/tr-registry.yaml`. Look up the story's TR-ID.
Read the current `requirement` text вЂ” this is the source of truth for what the
GDD requires now. Do not rely on any inline text in the story file (may be stale).

### The governing ADR
Read `design/architecture/[adr-file].md`. Extract:
...
Read `design/architecture/control-manifest.md`. Extract the rules for this story's layer:
- Required patterns
- Forbidden patterns
...
- Options:
  ...
  - `[B] Implement with old rules вЂ” I accept the risk of non-compliance`
  - `[C] Stop here вЂ” I want to review the manifest diff first`
...
1. Glob `.ags/project/epics/**/*.md` to find each dependency story file.
...
3. If any dependency has Status other than `Complete` or `Done`:
   - Use `AskUserQuestion`:
     ...
     - Options:
       - `[A] Proceed anyway вЂ” I accept the dependency risk`
       - `[B] Stop вЂ” I'll complete the dependency first`
       - `[C] The dependency is done but status wasn't updated вЂ” mark it Complete and continue`
   ...
   - If [A]: note in Phase 6 summary under "Deviations": "Implemented with incomplete dependency: [dependency title] вЂ” [status]."
...
- `Engine:` value вЂ” determines which programmer agents to use
...
**Config/Data stories вЂ” skip agent spawning entirely:**
If the story's Type is `Config/Data`, no programmer agent or engine specialist is needed. Jump directly to Phase 4 (Config/Data note). The implementation is a data file edit вЂ” no routing table evaluation, no engine specialist.

### Primary agent routing table

| Story context | Primary agent |
|---|---|
| Foundation layer вЂ” any type | `engine-programmer` |
| Any layer вЂ” Type: UI | `ui-programmer` |
| Any layer вЂ” Type: Visual/Feel | `gameplay-programmer` (implements) |
| Core or Feature вЂ” gameplay mechanics | `gameplay-programmer` |
| Core or Feature вЂ” AI behaviour, pathfinding | `ai-programmer` |
| Core or Feature вЂ” networking, replication | `network-programmer` |
| Config/Data вЂ” no code | No agent needed (see Phase 4 Config note) |

### Engine specialist вЂ” always spawn as secondary for code stories

Read the `Engine Specialists` section of `.ags/rules/technical-preferences.md`
...
|--------|----------------------------|
| Unity | `unity-specialist` (general + shader/addressables/UI), `unity-dots-specialist` (ECS/Jobs/Burst) |
...
3. The ADR Decision + Implementation Guidelines (verbatim вЂ” do not summarise)
...
Visual/Feel acceptance criteria cannot be auto-verified вЂ” the "does it feel right?"
...
this implementation вЂ” not deferred to later.
...
"Evidence doc required at `.ags/project/qa/evidence/[slug]-evidence.md`."
...
**Files changed**:
- `src/[path]` вЂ” created / modified ([brief description])
- `tests/[path]` вЂ” test file ([N] test functions)

**Acceptance criteria covered**:
- [x] [criterion] вЂ” implemented in [file:function]
- [x] [criterion] вЂ” covered by test [test_name]
- [ ] [criterion] вЂ” DEFERRED: requires playtest (Visual/Feel)
...
- Story: [story-path] вЂ” [story title]
- Files changed: [comma-separated list]
- Test written: [path, or "None вЂ” Visual/Feel/Config story"]
...
- Manifest version mismatch в†’ show diff to user, ask whether to proceed with old rules or update story first

## Collaborative Protocol

- **File writes are delegated** вЂ” all source code, test files, and evidence docs are written by sub-agents spawned via Task. Each sub-agent enforces the "May I write to [path]?" protocol individually. This orchestrator does not write files directly.
- **Load before implementing** вЂ” do not start coding until all context is loaded
  (story, TR-ID, ADR, manifest, engine prefs). Incomplete context produces code
  that drifts from design.
- **The ADR is the law** вЂ” implementation must follow the ADR's Implementation
  Guidelines. If the guidelines conflict with what seems "better," flag it in the
  summary rather than silently deviating.
- **Stay in scope** вЂ” the Out of Scope section is a contract. If implementing
...
- **Test is not optional for Logic/Integration** вЂ” do not mark implementation
  complete without the test file existing
- **Visual/Feel criteria are deferred, not skipped** вЂ” mark them as DEFERRED
  in the summary; they will be manually verified in `/story-done`
- **Ask before large structural decisions** вЂ” if the story requires an
