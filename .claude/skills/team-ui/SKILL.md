...
## Team Composition
- **ux-designer** вЂ” User flows, wireframes, accessibility, input handling
- **ui-programmer** вЂ” UI framework, screens, widgets, data binding, implementation
- **art-director** вЂ” Visual style, layout polish, consistency with art bible
- **engine UI specialist** вЂ” Validates UI implementation patterns against engine-specific best practices (read from `.ags/rules/technical-preferences.md` Engine Specialists в†’ UI Specialist)
- **ux-designer** вЂ” Audits accessibility compliance at Phase 4

**Templates used by this pipeline:**
- `ux-spec.md` вЂ” Standard screen/flow UX specification
- `hud-design.md` вЂ” HUD-specific UX specification
- `interaction-pattern-library.md` вЂ” Reusable interaction patterns
- `accessibility-requirements.md` вЂ” Committed accessibility tier and requirements

## How to Delegate

Use the Task tool to spawn each team member as a subagent:
- `subagent_type: ux-designer` вЂ” User flows, wireframes, accessibility, input handling
- `subagent_type: ui-programmer` вЂ” UI framework, screens, widgets, data binding
- `subagent_type: art-director` вЂ” Visual style, layout polish, art bible consistency
- `subagent_type: unity-specialist` вЂ” Engine-specific UI pattern validation (UI Toolkit / UGUI patterns, asset pipeline, performance)
- `subagent_type: ux-designer` вЂ” Accessibility compliance audit
...
- `design/gdd/concept.md` вЂ” platform targets and intended audience
- `design/player-journey.md` вЂ” player's state and context when they reach this screen
- All GDD UI Requirements sections relevant to this feature
- `design/ux/interaction-patterns.md` вЂ” existing patterns to reuse (not reinvent)
- `design/accessibility-requirements.md` вЂ” committed accessibility tier (e.g., Basic, Enhanced, Full)

**If `design/ux/interaction-patterns.md` does not exist**, surface the gap immediately:
> "interaction-patterns.md does not exist вЂ” no existing patterns to reuse."

Then use `AskUserQuestion` with options:
- (a) Run `/ux-design patterns` first to establish the pattern library, then continue
- (b) Proceed without the pattern library вЂ” ui-programmer will treat all patterns created as new and add each to a new `design/ux/interaction-patterns.md` at completion
...
**Gate**: Do not proceed to Phase 2 until the verdict is APPROVED. If the verdict is NEEDS REVISION, the ux-designer must address the flagged issues and re-run the review. The user may explicitly accept a NEEDS REVISION risk and proceed, but this must be a conscious decision вЂ” present the specific concerns via `AskUserQuestion` before asking whether to proceed.

### Phase 2: Visual Design

Delegate to **art-director**:
- Review the full UX spec (flows, wireframes, interaction patterns, accessibility notes) вЂ” not just the wireframe images
...
- Specify all asset requirements needed from the art pipeline: icons at specified sizes, background textures, fonts, decorative elements вЂ” with precise dimensions and format requirements
- Ensure consistency with existing implemented UI screens
- Output: visual design spec with style notes and asset manifest

### Phase 3: Implementation

Before implementation begins, spawn the **engine UI specialist** (from `.ags/rules/technical-preferences.md` Engine Specialists в†’ UI Specialist) to review the UX spec and visual design spec for engine-specific implementation guidance:
- Which Unity UI framework should be used for this screen? (e.g., UI Toolkit (UXML/USS) for editor-style and runtime UI vs UGUI Canvas for legacy/world-space integration)
...
- **Use patterns from `design/ux/interaction-patterns.md`** вЂ” do not reinvent patterns that are already specified. If a pattern almost fits but needs modification, note the deviation and flag it for ux-designer review.
- **UI NEVER owns or modifies game state** вЂ” display only; emit events for all player actions
- All text through the localization system вЂ” no hardcoded player-facing strings
...
- **Verify `design/ux/interaction-patterns.md` is up to date** вЂ” if any new patterns were introduced during this feature's implementation, confirm they have been added to the library
...
## Quick Reference вЂ” When to Use Which Skill

- `/ux-design` вЂ” Author a new UX spec for a screen, flow, or HUD from scratch
- `/ux-review` вЂ” Validate a completed UX spec before implementation
- `/team-ui [feature]` вЂ” Full pipeline from concept through polish (calls `/ux-design` and `/ux-review` internally)
- `/quick-design` вЂ” Small UI changes that don't need a full new UX spec
...
Verdict: **COMPLETE** вЂ” UI feature delivered through full pipeline (UX spec в†’ visual в†’ implementation в†’ review в†’ polish).
Verdict: **BLOCKED** вЂ” pipeline halted; surface the blocker and its phase before stopping.
