...
**If argument starts with `retrofit` + file path** (e.g., `/architecture-decision retrofit design/architecture/adr-0001-event-system.md`):
...
7. Suggest: "Run `/architecture-review` to re-validate coverage."
...
6. **Display knowledge gap warning** if domain MEDIUM or HIGH risk:
   ...
   No engine configured: prompt "No engine configured. Run `/setup-engine` first, or tell me which engine."
...
4.5. **Engine Specialist Validation** — Before saving, spawn **primary engine specialist** via Task to validate drafted ADR:
   ...
   - **Blocking issue** (wrong API, deprecated, version incompat): revise Decision + Engine Compatibility, confirm changes with user before proceeding
   - **Minor notes**: incorporate into Risks subsection

**Review mode check** before TD-ADR:
- `solo` → skip. Note: "TD-ADR skipped — Solo mode." → Step 4.7.
- `lean` → skip (not PHASE-GATE). Note: "TD-ADR skipped — Lean mode." → Step 4.7.
- `full` → spawn.

4.6. **Technical Director Strategic Review** — After specialist validation, spawn `technical-director` via Task using gate **TD-ADR** (`.ags/rules/director-gates.md`):
   ...
   - CONCERNS or REJECT: revise Decision/Alternatives before proceeding
...
No remaining priority ADRs and no undesigned GDDs: offer "Stop here" only, suggest `/architecture-review` in fresh session.
