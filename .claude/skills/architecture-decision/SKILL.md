...
## 0. Parse Arguments вЂ” Detect Retrofit Mode
...
(e.g., `/architecture-decision retrofit design/architecture/adr-0001-event-system.md`):

Enter **retrofit mode**:

1. Read the existing ADR file completely.
2. Identify which template sections are present by scanning headings:
   - `## Status` вЂ” **BLOCKING if missing**: `/story-readiness` cannot check ADR acceptance
   - `## ADR Dependencies` вЂ” HIGH if missing: dependency ordering breaks
   - `## Engine Compatibility` вЂ” HIGH if missing: post-cutoff risk unknown
   - `## GDD Requirements Addressed` вЂ” MEDIUM if missing: traceability lost
3. Present to the user:
   ...
   вњ“ Status: [current value, or "MISSING вЂ” will add"]
   вњ“ [section]

   Missing sections to add:
   вњ— Status вЂ” BLOCKING (stories cannot validate ADR acceptance without this)
   вњ— ADR Dependencies вЂ” HIGH
   вњ— Engine Compatibility вЂ” HIGH
   ```
4. Ask: "Shall I add the [N] missing sections? I will not modify any existing content."
5. If yes:
   - For **Status**: ask the user вЂ” "What is the current status of this decision?"
     Options: "Proposed", "Accepted", "Deprecated", "Superseded by ADR-XXXX"
   - For **ADR Dependencies**: ask вЂ” "Does this decision depend on any other ADR?
   ...
   - For **GDD Requirements Addressed**: ask вЂ” "Which GDD systems motivated this decision?
...
1. Read `.ags/docs/engine-reference/[engine]/VERSION.md` to get:
...
3. Read the corresponding module reference if it exists:
   `.ags/docs/engine-reference/[engine]/modules/[domain].md`

4. Read `.ags/docs/engine-reference/[engine]/breaking-changes.md` вЂ” flag any
   changes in the relevant domain that post-date the LLM's training cutoff.

5. Read `.ags/docs/engine-reference/[engine]/deprecated-apis.md` вЂ” flag any APIs
...
6. **Display a knowledge gap warning** before proceeding if the domain carries
   ...
   ```
   вљ пёЏ  ENGINE KNOWLEDGE GAP WARNING
   Engine: [name + version]
   Domain: [domain]
   Risk Level: HIGH вЂ” This version is post-LLM-cutoff.
   ...
   Proceed with verified information only вЂ” do NOT rely solely on training data.
   ```
...
Scan `design/architecture/` for existing ADRs to find the next number.
...
State Ownership:
  player_health в†’ owned by health-system (ADR-0001)
  Interface: HealthComponent.current_health (read-only float)
  в†’ If this ADR reads or writes player health, it must use this interface.

Interface Contracts:
  damage_delivery в†’ signal pattern (ADR-0003)
  Signal: damage_dealt(amount, target, is_crit)
  в†’ If this ADR delivers or receives damage events, it must use this signal.

Forbidden Patterns:
  вњ— autoload_singleton_coupling (ADR-0001)
  вњ— direct_cross_system_state_write (ADR-0000)
  в†’ The proposed approach must not use these patterns.
```

If the user's proposed decision would contradict any registered stance, surface
the conflict immediately:

> "вљ пёЏ Conflict: This ADR proposes [X], but ADR-[NNNN] established that [Y] is
...
a **confirm/adjust** prompt using `AskUserQuestion` вЂ” not open-ended questions.
...
[A] Proceed вЂ” draft with these assumptions
...
[C] Different approach вЂ” I'll describe it
```

**ADR Dependencies** вЂ” derive from existing ADRs, then confirm:
...
| **Knowledge Risk** | [LOW / MEDIUM / HIGH вЂ” from VERSION.md] |
...
4.5. **Engine Specialist Validation** вЂ” Before saving, spawn the **primary engine specialist** via Task to validate the drafted ADR:
...
**Review mode check** вЂ” apply before spawning TD-ADR:
- `solo` в†’ skip. Note: "TD-ADR skipped вЂ” Solo mode." Proceed to Step 4.7 (GDD sync check).
- `lean` в†’ skip (not a PHASE-GATE). Note: "TD-ADR skipped вЂ” Lean mode." Proceed to Step 4.7 (GDD sync check).
- `full` в†’ spawn as normal.

4.6. **Technical Director Strategic Review** вЂ” After the engine specialist validation, spawn `technical-director` via Task using gate **TD-ADR** (`.ags/rules/director-gates.md`):
   - Pass: the ADR file path (or draft content), engine version, domain, any existing ADRs in the same domain
   - The TD validates architectural coherence (is this decision consistent with the whole system?) вЂ” distinct from the engine specialist's API-level check
   - If CONCERNS or REJECT: revise the Decision or Alternatives sections accordingly before proceeding

4.7. **GDD Sync Check** вЂ” Before presenting the write approval, scan all GDDs
...
immediately before the write approval вЂ” not as a footnote:

```
вљ пёЏ GDD SYNC REQUIRED
[gdd-filename].md uses names this ADR has renamed:
  [old_name] в†’ [new_name_from_adr]
  [old_name_2] в†’ [new_name_2_from_adr]
...
- Patterns it bans (Consequences в†’ Negative or explicit "do not use X")

Present candidates:
```
Registry candidates from this ADR:
  NEW state ownership:      player_stamina в†’ stamina-system
  ...
  EXISTING (referenced_by update only): player_health в†’ already registered вњ…
```
...
3. Append the new entry AFTER the last existing entry in that section вЂ” do not try to replace a `[]` placeholder that may no longer exist
...
**BLOCKING вЂ” do not write to `docs/registry/architecture.yaml` without explicit user approval.**

Ask using `AskUserQuestion`:
- "May I update `docs/registry/architecture.yaml` with these [N] new stances?"
  - Options: "Yes вЂ” update the registry", "Not yet вЂ” I want to review the candidates", "Skip registry update"

Only proceed if the user selects yes. If yes: append new entries. Never modify existing entries вЂ” if a stance is
...
1. Read `docs/registry/architecture.yaml` вЂ” check if any priority ADRs are still unwritten (look for ADRs flagged in technical-preferences.md or systems-index.md as prerequisites)
2. Check if all prerequisite ADRs are now written. If yes, include a "Start writing GDDs" option.
3. List ALL remaining priority ADRs as individual options вЂ” not just the next one or two.

Widget format:
```
ADR-[NNNN] written and registry updated. What would you like to do next?
[1] Write [next-priority-adr-name] вЂ” [brief description from prerequisites list]
[2] Write [another-priority-adr] вЂ” [brief description]  (include ALL remaining ones)
[N] Start writing GDDs вЂ” run `/design-system [first-undesigned-system]` (only show if all prerequisite ADRs are written)
