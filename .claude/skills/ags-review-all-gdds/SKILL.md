...
**Distinct from `/ags-design-review`** (one GDD, internal completeness). This reviews
...
- Before `/ags-create-architecture` begins (architecture built on inconsistent GDDs
...
Run `/ags-consistency-check` after this review to populate the registry."
...
> then re-run `/ags-review-all-gdds`."
...
append to `.ags/project/state.md`:
    ...
    ## Session Extract — /ags-review-all-gdds [date]
    - Verdict: [PASS / CONCERNS / FAIL]
    - GDDs reviewed: [N]
...
- Are any GDDs in the "Flagged for Revision" table? → offer /ags-design-review option for each
- Read systems-index.md for the next system with Status: Not Started → offer /ags-design-system option
- Is the verdict PASS or CONCERNS? → offer /ags-gate-check or /ags-create-architecture
...
- `[_] Run /ags-design-review [flagged-gdd-path] — address flagged warnings` (one per flagged GDD, if any)
- `[_] Run /ags-design-system [next-system] — next in design order` (always include, name the actual system)
- `[_] Run /ags-create-architecture — begin architecture (verdict is PASS/CONCERNS)` (include if verdict is not FAIL)
- `[_] Run /ags-gate-check — validate Systems Design phase gate` (include if verdict is PASS)
- `[_] Stop here`
