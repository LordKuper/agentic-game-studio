...
`/ags-smoke-check` verifies test files **exist** and **pass**. This skill reviews
**quality** — a passing test may leave critical behaviour uncovered; a manual
evidence doc may lack required sign-offs.
...
- For BLOCKING items: "These must be resolved before `/ags-story-done` can mark the
