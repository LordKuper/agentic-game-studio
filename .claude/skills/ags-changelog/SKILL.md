...
Read sprint reports from `.ags/project/epics/` for the relevant period to understand planned work and context behind changes.
...
- Use `/ags-patch-notes [version]` to generate a styled, saved version for public release.
- Use `/ags-release-checklist` before publishing the changelog externally.
...
2. Persist current draft to `.ags/project/reviews/.tmp/changelog-[version]-iter[N]-draft.md`.
3. **Spawn in one message, in parallel**:
   - All internal reviewer Tasks (producer + narrative-director for tone).
...
4. Aggregator (`producer`) merges findings from internal + Codex, drops nitpicks + below-floor.
