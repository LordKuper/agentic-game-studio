...
- **No argument**: `/ags-map-systems` — Run the full decomposition workflow (Phases 1-5)
...
- Read `design/gdd/game-concept.md` — **fail with a clear message if missing**:
  > "No game concept found at `design/gdd/game-concept.md`. Run `/ags-brainstorm` first
...
Using the template at `.ags/templates/systems-index.md`, populate the
...
## Phase 6: Design Individual Systems (Handoff to /ags-design-system)

This phase is entered when:
- The user says "yes" to designing systems after creating the index
- The user invokes `/ags-map-systems [system-name]`
- The user invokes `/ags-map-systems next`
...
### Step 6b: Hand Off to /ags-design-system
...
- Runs `/ags-design-review` when complete
- Updates the systems index

**Do not duplicate the /ags-design-system workflow here.** This skill owns the systems
...
- "Systems index is written. What would you like to do next?"
  - [A] Start designing GDDs — run `/ags-design-system [first-system-in-order]`
...
- "Run `/ags-design-review design/gdd/[system].md` in a fresh session to validate quality"
- "Run `/ags-gate-check systems-design` when all MVP GDDs are complete"
...
1. **AskUserQuestion** at every decision point:
   ...
   - Phase 6: "Start designing, pick different, or stop?" → hand off to `/ags-design-system`
2. **"May I write to [filepath]?"** before every file write
3. **Incremental writing** — update systems index after each system designed
4. **Handoff** — GDD authoring owned by `/ags-design-system`; this skill owns the index
...
> **Context approaching limit (≥70%).** Systems index saved to `design/gdd/systems-index.md`. Open fresh session, run `/ags-map-systems next` to continue.

---

## Recommended Next Steps

- Run `/ags-design-system [first-system-in-order]` to author the first GDD (use design order from the index)
- Run `/ags-map-systems next` to always pick the highest-priority undesigned system automatically
- Run `/ags-design-review design/gdd/[system].md` in a fresh session after each GDD is authored
