...
- If it exists: read it, find the first context (system/level/character) with any asset at status "Needed" but no spec file written yet, and use `AskUserQuestion`:
  ...
  - Options: `[A] Yes вЂ” spec [target]` / `[B] Pick a different target` / `[C] Stop here`
- If no manifest: fail with:
  > "Usage: `/asset-spec system:<name>` вЂ” e.g., `/asset-spec system:tower-defense`
...
- `lean`: spawn `art-director` only вЂ” faster, skips technical constraint pass
- `solo`: no agent spawning вЂ” main session writes specs from art bible rules alone. Use for simple asset categories or when speed matters more than depth.
...
### Required reads:
- **Art bible**: Read `design/art/art-bible.md` вЂ” fail if missing:
  > "No art bible found. Run `/art-bible` first вЂ” asset specs are anchored to the art bible's visual rules and asset standards."
  Extract: Visual Identity Statement, Color System (semantic colors), Shape Language, Asset Standards (Section 8 вЂ” dimensions, formats, polycount budgets, texture resolution tiers).

- **Technical preferences**: Read `.ags/rules/technical-preferences.md` вЂ” extract performance budgets and naming conventions.
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  ...
  Use `AskUserQuestion`: `[A] Describe needs manually` / `[B] Stop вЂ” complete the GDD first`
...
### Optional reads:
- **Existing manifest**: Read `design/assets/asset-manifest.md` if it exists вЂ” extract already-specced assets for this target to avoid duplicates.
- **Related specs**: Glob `design/assets/specs/*.md` вЂ” scan for assets that could be shared (e.g., a common UI element specced for one system might apply here too).

### Present context summary:
> **Asset Spec: [Target Type] вЂ” [Target Name]**
> - Source doc: [path] вЂ” [N] asset types identified
> - Art bible: found вЂ” Asset Standards at Section 8
...
From the source doc, extract every asset type mentioned вЂ” explicit and implied.
...
- **Sprite / 2D Art** вЂ” character sprites, UI icons, tile sheets
- **VFX / Particles** вЂ” hit effects, ambient particles, screen effects
- **Environment** вЂ” props, tiles, backgrounds, skyboxes
- **UI** вЂ” HUD elements, menu art, fonts (if custom)
- **Audio** вЂ” SFX, music tracks, ambient loops *(note: audio specs are descriptions only вЂ” no generation prompts)*
- **3D Assets** вЂ” meshes, materials (if applicable per engine)
...
- Options: `[A] Proceed вЂ” spec all of these` / `[B] Remove some assets` / `[C] Add assets I didn't catch` / `[D] Adjust categories`
...
Spawn specialist agents based on review mode. **Issue all Task calls simultaneously вЂ” do not wait for one before starting the next.**

### Full mode вЂ” spawn in parallel:
...
- Ask: "For each asset in this list, produce: (1) a 2вЂ“3 sentence visual description anchored to the art bible's shape language and color system вЂ” be specific enough that two different artists would produce consistent results; (2) a generation prompt ready for use with AI image tools (Midjourney/Stable Diffusion style вЂ” include style keywords, composition, color palette anchors, negative prompts); (3) which art bible rules directly govern this asset (cite by section). For audio assets, describe the sonic character instead of a generation prompt."
...
- Ask: "For each asset in this list, specify: (1) exact dimensions or polycount (match the art bible Asset Standards tiers вЂ” do not invent new sizes); (2) file format and export settings; (3) naming convention (from technical-preferences.md); (4) any engine-specific constraints this asset type must respect; (5) LOD requirements if applicable. Flag any asset type where the art bible's preferred standard conflicts with the engine's constraints."

### Lean mode вЂ” spawn art-director only (skip technical-artist).

### Solo mode вЂ” skip both. Derive specs from art bible rules alone, noting that technical constraints were not validated.

**Collect both responses before Phase 4.** If any conflict exists between art-director and technical-artist (e.g., art-director specifies 4K textures but technical-artist flags the engine budget requires 512px), surface it explicitly вЂ” do NOT silently resolve.
...
```
## ASSET-[NNN] вЂ” [Asset Name]

| Field | Value |
|-------|-------|
| Category | [Sprite / VFX / Environment / UI / Audio / 3D] |
| Dimensions | [e.g. 256Г—256px, 4-frame sprite sheet] |
| Format | [PNG / SVG / WAV / etc.] |
| Naming | [e.g. vfx_frost_hit_01.png] |
| Polycount | [if 3D вЂ” e.g. <800 tris] |
| Texture Res | [e.g. 512px вЂ” matches Art Bible В§8 Tier 2] |

**Visual Description:**
[2вЂ“3 sentences. Specific enough for two artists to produce consistent results.]

**Art Bible Anchors:**
- В§3 Shape Language: [relevant rule applied]
- В§4 Color System: [color role вЂ” e.g. "uses Threat Blue per semantic color rules"]
...
- Prompt: "Asset specs for **[target]** вЂ” [N] assets. Review complete?"
- Options: `[A] Approve all вЂ” write to file` / `[B] Revise a specific asset` / `[C] Regenerate with different direction`

If [B]: ask which asset and what to change. Revise inline and re-present. Do NOT re-spawn agents for minor text revisions вЂ” only re-spawn if the visual direction itself needs to change.
...
```markdown
# Asset Specs вЂ” [Target Type]: [Target Name]
...
- Options:
  - `[A] Spec another system вЂ” /asset-spec system:[next-system]`
  - `[B] Spec a level вЂ” /asset-spec level:[level-name]`
  - `[C] Spec a character вЂ” /asset-spec character:[character-name]`
  - `[D] Run /asset-audit вЂ” validate delivered assets against specs`
  - `[E] Stop here`
...
Asset IDs are assigned sequentially across the entire project вЂ” not per-context. Read the manifest before assigning IDs to find the current highest number:
...
> "ASSET-012 (Generic Hit Spark) already specced for Combat system. Reusing for Tower Defense вЂ” adding tower-defense to referenced-by."
...
1. Surface immediately: "[AgentName]: BLOCKED вЂ” [reason]"
2. In `lean` mode or if `technical-artist` blocks: proceed with art-director output only вЂ” note that technical constraints were not validated
3. In `solo` mode or if `art-director` blocks: derive descriptions from art bible rules вЂ” flag as "Art director not consulted вЂ” verify against art bible before production"
4. Always produce a partial spec вЂ” never discard work because one agent blocked

---

## Collaborative Protocol

Every phase follows: **Identify в†’ Confirm в†’ Generate в†’ Review в†’ Approve в†’ Write**

- Never spec assets without first confirming the asset list with the user
- Always anchor specs to the art bible вЂ” a spec that contradicts the art bible is wrong
- Surface all agent disagreements вЂ” do not silently pick one
