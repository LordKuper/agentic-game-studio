...
- Generate the release checklist using `/ags-release-checklist`
...
- **tools-programmer**: Build release artifacts for all target platforms. Verify builds are clean and reproducible. Run automated tests in CI.
- **lead-programmer** *(if game has online features, multiplayer, or player data)*: Conduct pre-release security audit. Review authentication, anti-cheat, data privacy compliance. Sign off on security posture.
...
- Verify all strings are translated (delegate to **narrative-director** if available)
- Run performance benchmarks against targets (delegate to **performance-analyst** if available)
- **producer**: Verify all telemetry events fire correctly on release build. Confirm dashboards are receiving data. Check that critical funnels (onboarding, progression, monetization if applicable) are instrumented.
...
- Generate changelog using `/ags-changelog`
- Deploy to staging for final smoke test
- Deploy to production
- Monitor for 48 hours post-release

Delegate to **producer** (in parallel with deployment):
- Finalize patch notes using `/ags-patch-notes [version]`
...
- **producer**: Publish all player-facing communication, monitor community sentiment
- **producer**: Confirm live dashboards are healthy; alert if any critical events are missing
