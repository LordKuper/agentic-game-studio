...
- **`solo`**: Phases 1-4 only, no delegation, no Phase 5 next-step prompt вЂ” use when called from within another skill
...
**Dependency graph validation:** For every system listed in the Dependencies section, use Glob to check whether its GDD file exists in `design/gdd/`. Flag any that don't exist yet вЂ” these are broken references that downstream authors will hit.

**Lore/narrative alignment:** If `design/gdd/concept.md` or any file in `design/narrative/` exists, read it. Note any mechanical choices in this GDD that contradict established world rules, tone, or design pillars. Pass this context to `game-designer` in Phase 3b.

**Prior review check:** Check whether `design/gdd/reviews/[doc-name]-review-log.md` exists. If it does, read the most recent entry вЂ” note what verdict was given and what blocking items were listed. This session is a re-review; track whether prior items were addressed.
...
> "Full review: spawning specialist agents in parallel. This typically takes 8вЂ“15 minutes. Use `--review lean` for faster single-session analysis."

### Step 1 вЂ” Identify all domains the GDD touches

Read the GDD and identify every domain present. A GDD can touch multiple domains simultaneously вЂ” be thorough. Common signals:

| If the GDD contains... | Spawn these agents |
|------------------------|-------------------|
| Costs, prices, drops, rewards, economy | `systems-designer` |
| Combat stats, damage, health, DPS | `game-designer`, `systems-designer` |
| AI behaviour, pathfinding, targeting | `ai-programmer` |
| Level layout, spawning, wave structure | `game-designer` |
| Player progression, XP, unlocks | `systems-designer`, `game-designer` |
...
| Engine-specific patterns or APIs | Primary engine specialist (from `.ags/rules/technical-preferences.md`) |
...
### Step 2 вЂ” Spawn all relevant specialists in parallel

**CRITICAL: Task in this skill spawns a SUBAGENT вЂ” a separate independent Claude session
...
> Your job is NOT to validate this design вЂ” your job is to find problems.
...
- **`systems-designer`**: For every formula in the GDD, plug in boundary values (minimum and maximum plausible inputs). Report whether any outputs go degenerate вЂ” negative values, division by zero, infinity, or nonsensical results at the extremes.

- **`qa-lead`**: Review every acceptance criterion. Flag any that are not independently testable вЂ” phrases like "feels balanced", "works correctly", "performs well" are not ACs. Suggest concrete rewrites for any that fail this test.

### Step 3 вЂ” Senior lead review
...
### Step 4 вЂ” Surface disagreements
...
Re-review: [Yes вЂ” prior verdict was X on YYYY-MM-DD / No вЂ” first review]
...
- вњ“ enemy-definition-data.md вЂ” exists
- вњ— loot-system.md вЂ” NOT FOUND (file does not exist yet)

### Required Before Implementation
[Numbered list вЂ” blocking issues only. Each item tagged with source agent.]

### Recommended Revisions
[Numbered list вЂ” important but not blocking. Source-tagged.]

### Specialist Disagreements
[Any cases where agents disagreed with each other or with the main review.
Present both sides вЂ” do not silently resolve.]
...
- **S** вЂ” single system, no formulas, no new ADRs, <3 dependencies
- **M** вЂ” moderate complexity, 1-2 formulas, 3-6 dependencies
- **L** вЂ” multi-system integration, 3+ formulas, may require new ADR
- **XL** вЂ” cross-cutting concern, 5+ dependencies, multiple new ADRs likely
Label clearly: "Rough scope signal: M (producer should verify before sprint planning)"

### Verdict: [APPROVED / NEEDS REVISION / MAJOR REVISION NEEDED]
```

This skill is read-only вЂ” no files are written during Phase 4.
...
**First widget вЂ” what to do next:**

If APPROVED (first-pass, no revision needed), proceed directly to the systems-index widget, review-log widget, then the final closing widget. Do not show a separate "what to do" widget вЂ” the final closing widget covers next steps.

If NEEDS REVISION or MAJOR REVISION NEEDED, options:
- `[A] Revise the GDD now вЂ” address blocking items together`
- `[B] Stop here вЂ” revise in a separate session`
- `[C] Accept as-is and move on (only if all items are advisory)`

**If user selects [A] вЂ” Revise now:**

Work through all blocking items, asking for design decisions only where you cannot resolve the issue from the GDD and existing docs alone. Group all design-decision questions into a single multi-tab `AskUserQuestion` before making any edits вЂ” do not interrupt mid-revision for each blocker individually.

After all revisions are complete, show a summary table (blocker в†’ fix applied) and use `AskUserQuestion` for a **post-revision closing widget**:

- Prompt: "Revisions complete вЂ” [N] blockers resolved. What next?"
- Note current context usage: if context is above ~50%, add: "(Recommended: /clear before re-review вЂ” this session has used X% context. A full re-review runs 5 agents and needs clean context.)"
- Options:
  - `[A] Re-review in a new session вЂ” run /design-review [doc-path] after /clear`
  - `[B] Accept revisions and mark Approved вЂ” update systems index, skip re-review`
  - `[C] Move to next system вЂ” /design-system [next-system] (#N in design order)`
  - `[D] Stop here`

Never end the revision flow with plain text. Always close with this widget.

**Second widget вЂ” systems index update (always show this separately):**

Use a second `AskUserQuestion`:
- Prompt: "May I update `design/gdd/systems-index.md` to mark [system] as [In Review / Approved]?"
- Options: `[A] Yes вЂ” update it` / `[B] No вЂ” leave it as-is`

**Third widget вЂ” review log (always offer):**
...
- Options: `[A] Yes вЂ” append to review log` / `[B] No вЂ” skip`

If yes, append an entry in this format:
```
## Review вЂ” [YYYY-MM-DD] вЂ” Verdict: [APPROVED / NEEDS REVISION / MAJOR REVISION NEEDED]
...
**Final closing widget вЂ” always show after all file writes complete:**
...
- `design/gdd/systems-index.md` вЂ” find any system with Status: In Review or NEEDS REVISION (other than the one just reviewed)
- Count `.md` files in `design/gdd/` (excluding game-concept.md, systems-index.md) to determine if `/review-all-gdds` is worth offering (в‰Ґ2 GDDs)
- Find the next system with Status: Not Started in design order

Build the option list dynamically вЂ” only include options that are genuinely next:
- `[_] Run /design-review [other-gdd-path] вЂ” [system name] is still [In Review / NEEDS REVISION]` (include if another GDD needs review)
- `[_] Run /consistency-check вЂ” verify this GDD's values don't conflict with existing GDDs` (always include if в‰Ґ1 other GDD exists)
- `[_] Run /review-all-gdds вЂ” holistic design-theory review across all designed systems` (include if в‰Ґ2 GDDs exist)
