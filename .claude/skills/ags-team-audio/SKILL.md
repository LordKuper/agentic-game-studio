...
Spawn the **primary engine specialist** in parallel (from `.ags/rules/technical-preferences.md` Engine Specialists) to validate the integration approach:
- Is the proposed audio middleware integration idiomatic for Unity? (e.g., Unity Audio Mixer vs FMOD vs Wwise — match the project's middleware choice and review integration patterns accordingly)
...
- Use `/ags-dev-story` to implement the audio manager and event system once the design is approved.
- Run `/ags-asset-audit` after audio assets are created to verify naming and format compliance.
