...
| **Config/Data** | Smoke check pass report in `.ags/project/qa/smoke-*.md` | ADVISORY |
...
`tests/integration/[system]/` broadly, then check `.ags/project/session-logs/` for a
...
If none: flag as **ADVISORY** — "No smoke check report found. Run `/ags-smoke-check`."
...
2. **Manifest version staleness check**: Compare the `Manifest Version:` date
   ...
   - If the story's version is older → flag as ADVISORY:
     ...
     is v[current-date]. New rules may apply. Run /ags-story-readiness to check.`
...
`.ags/project/state.md`:
    ...
    ## Session Extract — /ags-story-done [date]
    - Verdict: [COMPLETE / COMPLETE WITH NOTES / BLOCKED]
...
Run `/ags-story-readiness [path]` to confirm a story is implementation-ready
before starting.
```
...
1. `/ags-smoke-check sprint` — verify the critical path still works end-to-end
2. `/ags-team-qa sprint` — full QA cycle: test case execution, bug triage, sign-off report
3. `/ags-gate-check` — advance to the next phase once QA approves

Do not run `/ags-gate-check` until `/ags-team-qa` returns APPROVED or APPROVED WITH CONDITIONS.
```
...
- Run `/ags-story-readiness [next-story-path]` to validate the next story before starting implementation
- If all Must Have stories are complete: run `/ags-smoke-check sprint` → `/ags-team-qa sprint` → `/ags-gate-check`
