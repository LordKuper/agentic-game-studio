...
- `subagent_type: game-designer` — Spatial layout, pacing, encounters, navigation
...
- `subagent_type: ux-designer` — Navigation clarity, colorblind safety, cognitive load
...
- (b) Pause and run `/ags-team-level [area-name]` first to establish that area
...
- Run `/ags-design-review design/levels/[level-name].md` to validate the completed level design doc.
- Run `/ags-dev-story` to implement level content once the design is approved.
- Run `/ags-qa-plan` to generate a QA test plan for this level.
