...
- **engine specialist** (primary) — Validate architecture and implementation patterns are idiomatic for the engine (read from `.ags/rules/technical-preferences.md` Engine Specialists section)
...
- Run `/ags-code-review` on the implemented combat code before closing stories.
- Run `/ags-balance-check` to validate combat formulas and tuning values.
- Run `/ags-team-polish` if VFX, audio, or performance polish is needed.
