...
- [ ] Smoke check passed (`/ags-smoke-check sprint`)
- [ ] QA sign-off report: APPROVED or APPROVED WITH CONDITIONS (`/ags-team-qa sprint`)
...
After generating a new sprint plan, also write `.ags/project/ags-sprint-status.yaml`.
This is the machine-readable source of truth for story status — read by
`/ags-sprint-status`, `/ags-story-done`, and `/ags-help` without markdown parsing.

Ask: "May I also write `.ags/project/ags-sprint-status.yaml` to track story status?"

Format:

```yaml
# Auto-generated by /ags-sprint-plan. Updated by /ags-story-done.
# DO NOT edit manually — use /ags-story-done to update story status.
...
After handling the producer's verdict, ask: "May I write this sprint plan to `.ags/project/sprints/sprint-[N].md`?" If yes, write the file, creating the directory if needed. Verdict: **COMPLETE** — sprint plan created. If no: Verdict: **BLOCKED** — user declined write.

After writing, add:

> **Scope check:** If this sprint includes stories added beyond the original epic scope, run `/ags-scope-check [epic]` to detect scope creep before implementation begins.
...
>
> Run `/ags-qa-plan sprint` now, before starting any implementation. It takes one session and produces the test case requirements each story needs."

Use `AskUserQuestion`:
- Prompt: "No QA plan found for this sprint. How do you want to proceed?"
- Options:
  - `[A] Run /ags-qa-plan sprint now — I'll do that before starting implementation (Recommended)`
  - `[B] Skip for now — I understand QA sign-off will be blocked at the Production → Polish gate`

If [A]: close with "Sprint plan written. Run `/ags-qa-plan sprint` next — then begin implementation."
If [B]: add a warning block to the sprint plan document:

```markdown
> ⚠️ **No QA Plan**: This sprint was started without a QA plan. Run `/ags-qa-plan sprint`
...
- `/ags-qa-plan sprint` — **required before implementation begins** — defines test cases per story so developers implement against QA specs, not a blank slate
- `/ags-story-readiness [story-file]` — validate a story is ready before starting it
- `/ags-dev-story [story-file]` — begin implementing the first story
- `/ags-sprint-status` — check progress mid-sprint
- `/ags-scope-check [epic]` — verify no scope creep before implementation begins
