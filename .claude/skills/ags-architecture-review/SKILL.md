...
Grep pattern="## Summary" glob="design/architecture/adr-*.md" output_mode="content" -A 3
```
...
### Engine Reference
- `.ags/docs/engine-reference/[engine]/VERSION.md`
- `.ags/docs/engine-reference/[engine]/breaking-changes.md`
- `.ags/docs/engine-reference/[engine]/deprecated-apis.md`
- All files in `.ags/docs/engine-reference/[engine]/modules/`

### Project Standards
- `.ags/rules/technical-preferences.md`
...
Before extracting any requirements, read `design/architecture/tr-registry.yaml`
...
If `design/architecture/architecture.md` exists, validate it against GDDs:
...
For each gap:
  ❌ TR-[id]: [GDD] → [system] → [requirement]
     Suggested ADR: "/ags-architecture-decision [suggested title]"
...
Matrix to `design/architecture/requirements-traceability.md`?"
...
> Last Updated: [date]
> Mode: /ags-architecture-review rtm
...
Also ask: "May I update `design/architecture/tr-registry.yaml` with new requirement
...
```markdown
### [YYYY-MM-DD] — /ags-architecture-review — 🔴 CONFLICT
...
`.ags/project/state.md`:
    ...
    ## Session Extract — /ags-architecture-review [date]
    - Verdict: [PASS / CONCERNS / FAIL]
...
2. **Gate guidance**: "When all blocking issues are resolved, run `/ags-gate-check
   pre-production` to advance"
3. **Rerun trigger**: "Re-run `/ags-architecture-review` after each new ADR is written
   to verify coverage improves"

Then close with `AskUserQuestion`:
- "Architecture review complete. What would you like to do next?"
  - [A] Write a missing ADR — open a fresh session and run `/ags-architecture-decision [system]`
  - [B] Run `/ags-gate-check pre-production` — if all blocking gaps are resolved
