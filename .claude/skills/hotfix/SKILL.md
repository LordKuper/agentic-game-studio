...
- **S1 (Critical)**: Game unplayable, data loss, security vulnerability вЂ” hotfix immediately
- **S2 (Major)**: Significant feature broken, workaround exists вЂ” hotfix within 24 hours
...
- `subagent_type: lead-programmer` вЂ” Review the fix for correctness and side effects
- `subagent_type: qa-lead` вЂ” Run targeted regression tests on the affected system
- `subagent_type: producer` вЂ” Approve deployment timing and communication plan

All three must return APPROVE before proceeding. If any returns CONCERNS or REJECT, do not deploy вЂ” surface the issue and resolve it first.
...
Apply the verdict:
- **Smoke check sufficient** вЂ” run `/smoke-check` against the hotfix build. If PASS, proceed to Phase 6.
- **Targeted QA pass required** вЂ” run `/team-qa [affected-system]` scoped to the changed system only. If QA returns APPROVED or APPROVED WITH CONDITIONS, proceed to Phase 6.
- **Full QA required** вЂ” S1 fixes that touch core systems may require a full `/team-qa sprint`. This delays deployment but prevents a bad patch.
...
```markdown
## Fix Record
**Fixed in**: hotfix/[branch-name] вЂ” [commit hash or description]
**Fixed date**: [date]
**Status**: Fixed вЂ” Pending Verification
```

Set `**Status**: Fixed вЂ” Pending Verification` in the bug file header.
...
**Approvals**: lead-programmer вњ“ / qa-lead вњ“ / producer вњ“
...
### Rules
- Hotfixes must be the MINIMUM change to fix the issue вЂ” no cleanup, no refactoring
...
If STILL PRESENT: the hotfix failed вЂ” immediately re-open, assess rollback, and escalate.
