...
> **Explicit invocation only**: Run only on `/hotfix`. No auto-invoke.
...
Apply verdict:
- **Smoke check sufficient** — `/smoke-check` against hotfix build. PASS → Phase 6.
- **Targeted QA required** — `/team-qa [affected-system]` scoped to changed system. APPROVED or APPROVED WITH CONDITIONS → Phase 6.
- **Full QA required** — S1 fixes touching core systems may need full `/team-qa sprint`. Delays deploy, prevents bad patch.
...
After deploy, run `/bug-report verify [BUG-ID]` to confirm fix resolved issue in deployed build.

VERIFIED FIXED → `/bug-report close [BUG-ID]`.
STILL PRESENT → hotfix failed; re-open, assess rollback, escalate.

Schedule post-incident review within 48 hours: `/retrospective hotfix`.
