...
Every shipped game has day-one patch. Plan it before launch to prevent chaos.
Scopes patch to only safe/necessary fixes, gates through lightweight QA, ensures
rollback plan exists before anything ships. Mini-sprint — not hotfix, not full sprint.
...
If [C]: output "No day-one patch required. Proceed to `/ags-launch-checklist`." Stop.
...
This is a lightweight QA pass — not a full `/ags-team-qa`. The patch is already QA-approved from the release gate; we are only re-verifying the changed areas.
...
- **Targeted smoke check** — run `/ags-smoke-check [affected-systems]`
...
- **Player communication is part of the patch** — `/ags-patch-notes` is a required output, not optional
