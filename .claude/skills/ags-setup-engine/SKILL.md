...
- **Version provided**: `/ags-setup-engine 6000.0.30f1` — pin this Unity version
- **No args**: `/ags-setup-engine` — fully guided mode (look up the latest stable Unity version, confirm with the user)
- **Refresh**: `/ags-setup-engine refresh` — update reference docs (see Section 10)
- **Upgrade**: `/ags-setup-engine upgrade [old-version] [new-version]` — migrate to a new Unity version (see Section 11)
...
> "No game concept found. Consider running `/ags-brainstorm` first to discover
...
Run `/ags-setup-engine refresh` to populate full reference docs at any time.
```
...
If invoked as `/ags-setup-engine refresh`:
...
If invoked as `/ags-setup-engine upgrade [old-version] [new-version]`:
...
2. Run /ags-setup-engine refresh after upgrading the actual Unity binary to
   verify no new deprecations were missed
3. Run /ags-architecture-review — the engine upgrade may invalidate ADRs that
   reference specific APIs or engine capabilities
4. If any ADRs are invalidated, run /ags-propagate-design-change to update
   downstream stories
```
