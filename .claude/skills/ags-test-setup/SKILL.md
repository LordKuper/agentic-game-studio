...
1. **Read engine config**:
   - Read `.ags/rules/technical-preferences.md` and extract the `Engine:` value.
   - If engine is not configured (`[TO BE CONFIGURED]`), stop:
     "Engine not configured. Run `/ags-setup-engine` first, then re-run `/test-setup`."
...
tests/
  ...
  smoke/          # Critical path test list for /ags-smoke-check gate
...
**Run via**: `/ags-smoke-check` (which reads this file)
...
3. Run `/ags-qa-plan sprint` before your first sprint to classify stories and set
   test evidence requirements
4. `/ags-smoke-check` before every QA hand-off

Gate note: /ags-gate-check Technical Setup → Pre-Production now requires:
...
- **Engine detection is non-negotiable** — if the engine is not configured,
  stop and redirect to `/ags-setup-engine`. Do not guess.
