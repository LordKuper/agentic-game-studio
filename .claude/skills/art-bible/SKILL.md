...
- `/map-systems` — decompose concept into game systems before authoring GDDs
- `/setup-engine` — if engine not yet configured (asset standards may need revisiting)
- `/design-system [first-system]` — start authoring per-system GDDs
- `/consistency-check` — validate GDDs against art bible once GDDs exist
- `/create-architecture` — produce master architecture document
