...
**Layout mirroring**
- Is RTL layout enabled in Unity? (TextMeshPro RTL Adjuster, UI Toolkit `direction: rtl` USS, or the Unity Localization package's direction settings)
...
- The list of all screens/flows in the game (from `design/gdd/` or `/ags-content-audit` output)
- The current `/ags-localize validate` report
...
```
/ags-localize scan            → find hardcoded strings
/ags-localize extract         → build string table
/ags-localize freeze          → lock source before sending to translators
/ags-localize brief           → generate translator briefing document
[Send to translators]
/ags-localize validate        → check returned translations
/ags-localize cultural-review → flag culturally sensitive content
/ags-localize rtl-check       → if shipping Arabic / Hebrew / Persian
/ags-localize vo-pipeline     → if shipping dubbed VO
/ags-localize qa              → full localization QA pass
```
