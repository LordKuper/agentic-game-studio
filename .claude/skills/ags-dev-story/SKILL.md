...
Bridges planning and code. Reads story file, assembles full context, routes to correct specialist, drives implementation to completion — including writing the test.

**The loop for every story:**
```
/ags-qa-plan sprint           ← define test requirements before sprint begins
/ags-story-readiness [path]   ← validate before starting
/ags-dev-story [path]         ← implement it  (this skill)
/ags-code-review [files]      ← review it
/ags-story-done [path]        ← verify and close it
```

**After all sprint stories are done:** run `/ags-team-qa sprint` to execute the full QA cycle and get a sign-off verdict before advancing the project stage.

**Output:** Source code + test file in the project's `Assets/Scripts/` and `tests/` directories.
...
|------|------|------------|
| TR registry | `design/architecture/tr-registry.yaml` | **STOP** — "TR registry not found. Run `/ags-create-epics` to generate it." |
| Governing ADR | path from story's ADR field | **STOP** — "ADR file [path] not found. Run `/ags-architecture-decision` to create it, or correct the filename in the story's ADR field." |
| Control manifest | `design/architecture/control-manifest.md` | **WARN and continue** — "Control manifest not found — layer rules cannot be checked. Run `/ags-create-control-manifest`." |
...
If [C]: stop. Do not spawn any agent. Let the user review and re-run `/ags-dev-story`.
...
check happens in `/ags-story-done` via manual confirmation.
...
> The story cannot be closed via `/ags-story-done` without it. Write the test
...
**Files changed**:
- `Assets/Scripts/[path]` — created / modified ([brief description])
...
Ready for: `/ags-code-review [file1] [file2]` then `/ags-story-done [story-path]`
```
...
```
## Session Extract — /ags-dev-story [date]
- Story: [story-path] — [story title]
- Files changed: [comma-separated list]
- Test written: [path, or "None — Visual/Feel/Config story"]
- Blockers: [None, or description]
- Next: /ags-code-review [files] then /ags-story-done [story-path]
```

Create `state.md` if it does not exist. Confirm: "Session state updated."
...
- ADR status is Proposed → do not implement; run `/ags-architecture-decision` first
- Scope too large → split into two stories via `/ags-create-stories`
...
- **File writes delegated** — source code, test files, evidence docs written by sub-agents via Task. This orchestrator does not write files directly.
- **Load before implementing** — all context (story, TR-ID, ADR, manifest, engine prefs) loaded before coding. Incomplete context → code drifts from design.
- **ADR is the law** — follow ADR Implementation Guidelines. Conflict with "better" approach → flag in summary, don't silently deviate.
- **Stay in scope** — Out of Scope is a contract. Need out-of-scope file → stop and surface: "Implementing [criterion] requires [file], which is out of scope. Proceed or create separate story?"
- **Test not optional for Logic/Integration** — don't mark complete without test file
- **Visual/Feel criteria deferred, not skipped** — mark DEFERRED; manually verified in `/ags-story-done`
- **Ask before structural decisions** — ADR doesn't cover [case] → "My plan is [X]. Proceed?"

---

## Recommended Next Steps

- Run `/ags-code-review [file1] [file2]` to review the implementation before closing the story
- Run `/ags-story-done [story-path]` to verify acceptance criteria and mark the story complete
- After all sprint stories are done: run `/ags-team-qa sprint` for the full QA cycle before advancing the project stage
