...
- **engine UI specialist** — Validates UI implementation patterns against engine-specific best practices (read from `.ags/rules/technical-preferences.md` Engine Specialists → UI Specialist)
...
- `subagent_type: unity-specialist` — Engine-specific UI pattern validation (UI Toolkit / UGUI patterns, asset pipeline, performance)
...
Before implementation begins, spawn the **engine UI specialist** (from `.ags/rules/technical-preferences.md` Engine Specialists → UI Specialist) to review the UX spec and visual design spec for engine-specific implementation guidance:
...
- Run `/ags-code-review` on the UI implementation before closing stories.
- Run `/ags-team-polish` if visual or audio polish pass is needed.
