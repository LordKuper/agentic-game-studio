...
- Read the latest milestone in `.ags/project/milestones/`
- Read any existing release checklist in `.ags/project/releases/`
...
Spawn `release-manager` and `qa-lead` via Task to review the drafted launch checklist against target platforms, cert profiles, QA sign-off, and prior incident history.
...
If not in dry-run mode, ask: "May I write this to `.ags/project/releases/launch-checklist-[date].md`?"
...
- Run `/ags-gate-check` to get a formal PASS/CONCERNS/FAIL verdict before launch.
