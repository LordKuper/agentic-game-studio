...
| **Polish** | Explicit only (set by `/ags-gate-check` Production → Polish gate) |
| **Release** | Explicit only (set by `/ags-gate-check` Polish → Release gate) |
...
- "I found a game concept but no systems index. Have you decomposed the concept into individual systems yet, or should we run `/ags-map-systems`?"
...
If user provided a role argument (e.g., `/ags-project-stage-detect programmer`):
...
/ags-project-stage-detect programmer

# Designer-focused analysis
/ags-project-stage-detect designer
```
