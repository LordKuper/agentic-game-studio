...
| Command | Purpose |
|---------|---------|
| `/ags-start` | First-time onboarding вЂ” asks where you are, then guides you to the right workflow |
| `/help` | Context-aware "what do I do next?" вЂ” reads current stage and surfaces the required next step |
| `/project-stage-detect` | Full project audit вЂ” detect phase, identify existence gaps, recommend next steps |
| `/setup-engine` | Configure engine + version, detect knowledge gaps, populate version-aware reference docs |
| `/adopt` | Brownfield format audit вЂ” checks internal structure of existing GDDs/ADRs/stories, produces migration plan |
...
| `/quick-design` | Lightweight design spec for small changes вЂ” tuning, tweaks, minor additions |
...
| Command | Purpose |
|---------|---------|
| `/create-epics` | Translate GDDs + ADRs into epics вЂ” one per architectural module |
...
| `/balance-check` | Analyze game balance data, formulas, and config вЂ” flag outliers |
