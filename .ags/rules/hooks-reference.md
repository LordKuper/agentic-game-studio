...
| `detect-gaps.sh` | SessionStart | Session begins | Detect fresh projects (suggest `/ags-start`); missing docs when code exists → suggest `/ags-reverse-document` or `/ags-project-stage-detect` |
...
| `validate-skill-change.sh` | PostToolUse (Write/Edit) | Skill file changes | Advise running `/ags-skill-test` after `.claude/skills/` write |
